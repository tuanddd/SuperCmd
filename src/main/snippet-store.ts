...
export function updateSnippet(
  id: string,
  data: Partial<Pick<Snippet, 'name' | 'content' | 'keyword'>>
): Snippet | null {
  if (!snippetsCache) snippetsCache = loadFromDisk();
...
export function resolveSnippetPlaceholders(content: string): string {
  const now = new Date();
...
interface SnippetExportFile {
  ...
  type: string;
  exportedAt: string;
  snippets: Array<{ name: string; content: string; keyword?: string }>;
}
...
export async function importSnippetsFromFile(parentWindow?: BrowserWindow): Promise<{ imported: number; skipped: number }> {
  ...
  try {
    ...
    let snippetsToImport: Array<{ name: string; content: string; keyword?: string }> = [];
